...
parsons_code <- function(df, string_col, global_head_col, group_head_col, individual_middle_col, random_variation_col, group_tail_col, global_tail_col, mapping = list("A" = "up", "B" = "down", "C" = "constant", "D"="double")) {
  ...
  if(!any(is.na(df[[group_head_col]])) & !any(is.na(df[[group_tail_col]])) & !any(is.na(df[[individual_middle_col]]))){
