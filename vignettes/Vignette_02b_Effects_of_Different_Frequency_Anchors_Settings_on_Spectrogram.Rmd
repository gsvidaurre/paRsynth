...
This vignette is very similar to Vignette 02 *BUT* with added steps exploring the effects of the two different `frequency_anchors` settings: "continuous_trajectory" and "starting_frequency". "continuous_trajectory"  uses the set `starting_frequency` parameter at the start of each vocalization component. In other words, if the starting frequency is set to 4000 Hz,then the `global_head` followed by `group_information`, `individual_information`, `random_variation`, and lastly `global_tail` would each section would start at a frequency of 4000 Hz. "starting_frequency" only uses the set `starting_frequency` parameter for the beginning of the vocalization, not after each component. This would mean that depending on the ending frequency of the last component would influence the starting frequency of the next.  
...
It is important to have your files organized, especially when creating many different types of output data (audios and images). Therefore, initializing working directories in the beginning will be helpful in understanding the data you create with paRsynth and where they are stored. You will need to adjust your `path` based on where you want your paRsynth outputs to be stored on your local machine.
