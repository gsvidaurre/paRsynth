...
sel_tblA <- data.table::rbindlist(pblapply(1:length(wavs), function(w){
  
  tmpA <- tuneR::readWave(file.path(audio_path, wavs[w]))
  
  # Return the metadata for the given call using the synthetic metadata generated during audio file generation above
  metadats_tmpA <- synthetic_call_metadata1_cf_csv %>%
  ...
  if(nrow(metadats_tmpA) > 0){
    ...
    res <- data.frame(
      ...
      selec = 1,
      start = 0.1, 
      end = seewave::duration(tmpA) - 0.1,
      sampling_rate = tmpA@samp.rate,
      group_ID = metadats_tmpA[["Group"]],
      individual_ID = metadats_tmpA[["Individual"]],
      call_ID = metadats_tmpA[["Call"]]
    )
...
# Iterate over the audio files to create one row of the selection table at a time. warbleR selection tables have a very specific format that must be used for downstream analysis.
sel_tblB <- data.table::rbindlist(pblapply(1:length(wavs), function(w){
  
  tmpB <- tuneR::readWave(file.path(audio_path, wavs[w]))
  
  # Return the metadata for the given call using the synthetic metadata generated during audio file generation above
  metadats_tmpB <- synthetic_call_metadata1_sf_csv %>%
    dplyr::filter(grepl(wavs[w], audio_file_name))
  
  # Create a row for the selection table in warbleR format if the audio file exists and metadata for that file also exists
  if(nrow(metadats_tmpB) > 0){
    ...
    res <- data.frame(
      ...
      selec = 1,
      start = 0.1, 
      end = seewave::duration(tmpB) - 0.1,
      sampling_rate = tmpB@samp.rate,
      group_ID = metadats_tmpB[["Group"]],
      individual_ID = metadats_tmpB[["Individual"]],
      call_ID = metadats_tmpB[["Call"]]
    )
...
Change the image path to reflect the spectrograms you have generated

<div style="display: flex;">
  <figure>
  <img src="/Users/gretheljuarez/Desktop/BIRDS/paRsynth_frequency_anchors_settings_comparison/images/A_Group1_Indiv1_Call1.jpeg" alt="Image 1" style="width: 400; margin-right: 10px;">
  <figcaption><b>continuous_trajectory</b>_Group1_Individual1_Call1</figcaption>
  </figure>
  <figure>
  <img src="/Users/gretheljuarez/Desktop/BIRDS/paRsynth_frequency_anchors_settings_comparison/images/B_Group1_Indiv1_Call1.jpeg" alt="Image 2" style="width: 400;">
...
<div style="display: flex;">
  <figure>
  <img src="/Users/gretheljuarez/Desktop/BIRDS/paRsynth_frequency_anchors_settings_comparison/images/A_Group2_Indiv5_Call2.jpeg" alt="Image 1" style="width: 400; margin-right: 10px;">
  <figcaption><b>continuous_trajectory</b>_Group2_Individual5_Call2</figcaption>
  </figure>
  <figure>
  <img src="/Users/gretheljuarez/Desktop/BIRDS/paRsynth_frequency_anchors_settings_comparison/images/B_Group2_Indiv5_Call2.jpeg" alt="Image 2" style="width: 400;">
