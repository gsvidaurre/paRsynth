...
In this vignette, we will go through how to create vocalization strings, convert them into Parsons code, calculate frequency anchors, and generate synthetic audio files using the paRsynth package for downstream bioacoustics analyses AND emphasizing the effects of the two different setting of 'frequency_anchors' on the spectrograms. This vigentte will be very similar to Vignette 02 BUT with added steps exploring the effects of the two different frequency_anchors settings: continuous_trajectory and starting_frequency.
...
analysis_dir <- "paRsynth_methods_synthetic_dataset_copy"
...
Additionally, from this step onward, two datasets will be made and used to explore the effects of the two different frequency_anchors setting of section_transition = "continuous_trajectory" versus section_transition = "starting_frequency" on the spectrograms of the paRsynth generated calls.
...
This will return an updated data frames that includes all of the previous columns in addition to new columns that represent each frequency anchor for each vocalization (the number of columns added in this step corresponds to the character length of your vocalization string).

Now we have two datasets and going to create calls and spectrograms for them to see how they differ.
...
synthetic_call_metadata1_cf <- write_audio(
...
glimpse(synthetic_call_metadata1_cf)
...
synthetic_call_metadata1_sf <- write_audio(
...
glimpse(synthetic_call_metadata1_sf)
...
synthetic_call_metadata1_cf %>%
  write.csv(., file = file.path(analysis_path, "synthetic_call_metadata1_cf.csv"), row.names = FALSE) 

```

```{r}

synthetic_call_metadata1_cf_csv <- read.csv(file.path(analysis_path, "synthetic_call_metadata1_cf.csv"))

glimpse(synthetic_call_metadata1_cf_csv)
...
# Add extra metadata for upcoming image files names and rearrange columns
synthetic_call_metadata1_sf %>%
  write.csv(., file = file.path(analysis_path, "synthetic_call_metadata1_sf.csv"), row.names = FALSE) 

```

```{r}

synthetic_call_metadata1_sf_csv <- read.csv(file.path(analysis_path, "synthetic_call_metadata1_sf.csv"))

glimpse(synthetic_call_metadata1_sf_csv)

```

**Additional Step 7: Combine datasets into one single CSV file**
...
Example_A <- read.csv("/Users/gretheljuarez/Desktop/BIRDS/paRsynth_methods_synthetic_dataset_copy/synthetic_call_metadata1_cf.csv")
Example_B <- read.csv("/Users/gretheljuarez/Desktop/BIRDS/paRsynth_methods_synthetic_dataset_copy/synthetic_call_metadata1_sf.csv")

# Combine the CSV files of each dataset
synthetic_call_metadata_cf_sf <- rbind(Example_A, Example_B)

# Write out the CSV file containing both datasets
synthetic_call_metadata_cf_sf %>%
  write.csv(., file = file.path(analysis_path, "synthetic_call_metadata_cf_sf.csv"), row.names = FALSE)

```

```{r}

synthetic_call_metadata_cf_sf_csv <- read.csv(file.path(analysis_path, "synthetic_call_metadata_cf_sf.csv"))

glimpse(synthetic_call_metadata_cf_sf_csv)

```
 
After running this code you will have two datasets combined into a single CSV: 1) reflects vocalizations created with continuous_trajectory setting and 2) vocalizations created with starting_frequency setting. Now we are ready to do some Bioacoustics Analysis steps to analyze and compare these outputs you created with paRsynth.
...
sel_tbl <- data.table::rbindlist(lapply(1:length(wavs), function(w){
  ...
  tmp <- tuneR::readWave(file.path(audio_path, wavs[w])) # Read the audio file
  
  # Return the metadata for the given call using the synthetic metadata generated during audio file generation above
  synthetic_call_metadata <- read.csv("/Users/gretheljuarez/Desktop/BIRDS/paRsynth_methods_synthetic_dataset_copy/synthetic_call_metadata_cf_sf.csv")
  
  # Return the metadata for the given call using the synthetic metadata generated during audio file generation above by filtering the metadata for the audio file
...
<div style="display: flex;">
  <figure>
  <img src="/Users/gretheljuarez/Desktop/BIRDS/paRsynth_methods_synthetic_dataset_copy/figures/A_Group1_Indiv1_Call1.png" alt="Image 1" style="width: 400; margin-right: 10px;">
  <figcaption><b>continuous_trajectory</b>_Group1_Individual1_Call1</figcaption>
  </figure>
  <figure>
  <img src="/Users/gretheljuarez/Desktop/BIRDS/paRsynth_methods_synthetic_dataset_copy/figures/B_Group1_Indiv1_Call1.png" alt="Image 2" style="width: 400;">
  <figcaption><b>starting_frequency</b>_Group1_Individual1_Call1</figcaption>
  </figure>
</div>

<div style="display: flex;">
  <figure>
  <img src="/Users/gretheljuarez/Desktop/BIRDS/paRsynth_methods_synthetic_dataset_copy/figures/A_Group2_Indiv5_Call2.png" alt="Image 1" style="width: 400; margin-right: 10px;">
  <figcaption><b>continuous_trajectory</b>_Group2_Individual5_Call2</figcaption>
  </figure>
  <figure>
  <img src="/Users/gretheljuarez/Desktop/BIRDS/paRsynth_methods_synthetic_dataset_copy/figures/B_Group2_Indiv5_Call2.png" alt="Image 2" style="width: 400;">
  <figcaption><b>starting_frequency</b>_Group2_Individual5_Call2</figcaption>
  </figure>
</div>
<br>
</br>
