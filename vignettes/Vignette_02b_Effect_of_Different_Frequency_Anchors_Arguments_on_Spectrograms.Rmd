...
This vignette is very similar to Vignette 02 *BUT* with added steps exploring the effects of the two different `frequency_anchors` argument settings: "continuous_trajectory" and "starting_frequency". "continuous_trajectory" uses the set `starting_frequency` parameter at the start of each vocalization component. In other words, if the starting frequency is set to 4000 Hz,then the `global_head` followed by `group_information`, `individual_information`, `random_variation`, and lastly `global_tail` would each start at a frequency of 4000 Hz and the pattern would be read until the start of the next section. "starting_frequency" only uses the set `starting_frequency` parameter for the beginning of the call and the pattern would continue all the way through the end of the call, not after each component. This would mean that depending on the ending frequency of the last pattern would influence the frequency of the next differently between both argument settings.  
...
calls_parsons_frequencies_a <- frequency_anchors(
  df = calls_parsons, 
...
glimpse(calls_parsons_frequencies_a)
...
calls_parsons_frequencies_b <- frequency_anchors(
  df = calls_parsons, 
  parsons_col = "Call_Parsons_Code", 
  group_id_col = "Group", 
  individual_id_col = "Individual", 
...
glimpse(calls_parsons_frequencies_b)
...
synthetic_call_metadata_a <- write_audio(
  df = calls_parsons_frequencies_a,
...
glimpse(synthetic_call_metadata_a)
...
synthetic_call_metadata_b <- write_audio(
  df = calls_parsons_frequencies_b,
  save_path = audio_path, 
  sampling_rate = 44100,
  sylLen = 200,
  temperature = 0.025,  # default stochasticity in generating sound
...
glimpse(synthetic_call_metadata_b)
...
synthetic_call_metadata_a %>%
  write.csv(., file = file.path(analysis_path, "synthetic_call_metadata_a.csv"), row.names = FALSE) 

```

```{r message = FALSE, warning = FALSE, eval = FALSE, echo = TRUE}

synthetic_call_metadata_a_csv <- read.csv(file.path(analysis_path, "synthetic_call_metadata_a.csv"))

glimpse(synthetic_call_metadata_a_csv)
...
# Add extra metadata for upcoming image files names and rearrange columns
synthetic_call_metadata_b %>%
  write.csv(., file = file.path(analysis_path, "synthetic_call_metadata_b.csv"), row.names = FALSE) 

```

```{r message = FALSE, warning = FALSE, eval = FALSE, echo = TRUE}

synthetic_call_metadata_b_csv <- read.csv(file.path(analysis_path, "synthetic_call_metadata_b.csv"))

glimpse(synthetic_call_metadata_b_csv)
...
synthetic_call_metadata_a_csv %>%
  dplyr::mutate(
    dataset = "A"
  ) %>% 
  bind_rows(
    synthetic_call_metadata_b_csv %>% 
      dplyr::mutate(
        dataset = "B"
      )
  ) %>%
