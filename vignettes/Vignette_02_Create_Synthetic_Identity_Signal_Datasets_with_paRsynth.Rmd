...
vignette: >
  %\VignetteIndexEntry{Vignette 02: Create Synthetic Identity Signal Datasets with `paRsynth`}
...
<br> We will create two distinct datasets: <br>
    1. more group information than individual<br>
    2. more individual information than group
...
path <- "/Users/raneemsamman/Desktop/BIRDS" # example
...
source(file.path("/Users/raneemsamman/Documents/GitHub/paRsynth/R/generate_strings.R"))
source(file.path("/Users/raneemsamman/Documents/GitHub/paRsynth/R/parsons_code.R"))
source(file.path("/Users/raneemsamman/Documents/GitHub/paRsynth/R/frequency_anchors.R"))
source(file.path("/Users/raneemsamman/Documents/GitHub/paRsynth/R/write_audio.R"))
...
synthetic_call_metadata_grpdom <- write_audio(
  df = calls_parsons_frequencies,
  save_path = "/Users/raneemsamman/Desktop/BIRDS/paRsynth_methods_synthetic_dataset/audio", 
  sampling_rate = 44100,
  sylLen = 200,
...
glimpse(synthetic_call_metadata_grpdom)
...
synthetic_call_metadata_grpdom %>%
  write.csv(., file = file.path(analysis_path, "synthetic_call_metadata_grpdom.csv"), row.names = FALSE) 

```

```{r message = FALSE, warning = FALSE, eval = FALSE, echo = TRUE}

synthetic_call_metadata_grpdomcsv <- read.csv(file.path(analysis_path, "synthetic_call_metadata_grpdom.csv"))

glimpse(synthetic_call_metadata_grpdomcsv)
...
group_information_inddom <- 2
individual_information_inddom <- 4

random_variation_inddom <- 2

globals_inddom <- 8
string_length_inddom <- group_information_inddom + individual_information_inddom + random_variation_inddom + globals_inddom

n_groups_inddom <- 2
n_individuals_inddom <- 10
n_calls_inddom <- 2
...
calls_inddom <- generate_strings(
  n_groups = n_groups_inddom, 
  n_individuals = n_individuals_inddom, 
  n_calls = n_calls_inddom, 
  string_length = string_length_inddom, 
  group_information = group_information_inddom, 
  individual_information = individual_information_inddom,
  random_variation = random_variation_inddom
  )

glimpse(calls_inddom)
```

```{r message = FALSE, warning = FALSE, eval = TRUE, echo = TRUE}

### Convert each string to Parsons code
calls_parsons_inddom <- parsons_code(
  df = calls_inddom, 
  string_col = "Call", 
  global_head_col = "Global_head",
  group_head_col = "Group_head",
...
calls_parsons_frequencies_inddom <- frequency_anchors(
  df = calls_parsons_inddom, 
  parsons_col = "Call_Parsons_Code", 
  group_id_col = "Group", 
  individual_id_col = "Individual", 
  call_id_col = "Call_ID", 
...
glimpse(calls_parsons_frequencies_inddom)
...
synthetic_call_metadata_inddom <- write_audio(
  df = calls_parsons_frequencies_inddom,
  save_path = audio_path, 
  sampling_rate = 44100,
  sylLen = 200,
  temperature = 0.025,  # default stochasticity in generating sound
...
glimpse(synthetic_call_metadata_inddom)
...
# Add extra metadata for upcoming image files names and rearrange columns
synthetic_call_metadata_inddom %>%
  write.csv(., file = file.path(analysis_path, "synthetic_call_metadata_inddom.csv"), row.names = FALSE)

```

```{r message = FALSE, warning = FALSE, eval = FALSE, echo = TRUE}

synthetic_call_metadata_inddomcsv <- read.csv(file.path(analysis_path, "synthetic_call_metadata_inddom.csv"))

glimpse(synthetic_call_metadata_inddomcsv)
