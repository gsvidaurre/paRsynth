...
vignette: >
  %\VignetteIndexEntry{Vignette 01: Setup and Introduction}
...
This vignette will guide you through the setup of the `paRsynth` package, introduce its purpose and functions before demonstrating how to utilize it for your research.
...
* Go to the tool bar above the R Studio window and select the option "Tools"
...
* Select the option "Code" on the left hand side

* Check the box next to the option "Soft-wrap R source files"

* Select "Apply" to apply those changes, then "OK" to exit 
...
* Click on the arrow of the green "Code" button on the top right 
...
`paRsynth` uses Parsons code to convert randomly generated character strings to frequency modulated vocalizations with different amounts of individual identity and group membership information. The functions in this package were developed for the sonification of character strings that represent vocal signals with varying amounts of social information. The resulting audio files (created using the soundgen package) can be used in downstream bioacoustics analyses. By using widely accepted bioacoustic analysis tools on these vocalizations, those tools can be evaluated for accuracy on social information detection.
...
`paRsynth` provides four functions for the simulation of vocalizations with representation of both individual identity and group membership information. These vocalizations are encoded as strings that can be used for generating synthetic audio signals for bioacoustic analysis. The next vignette (*Vignette 02: Create Vocal Identity Signals with `paRsynth`*) will demonstrate how to use the package's functions to generate vocalizations strings, convert them into Parsons code, calculate frequency anchors, and generate synthetic audio files.
...
![Figure 1. `paRsynth` functions workflow](./images/paRsynth_methods_workflow_horizontal_vignette.png)
