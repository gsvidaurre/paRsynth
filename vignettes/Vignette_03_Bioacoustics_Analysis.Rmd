...
![Example Spectrogram of GroupMembership_Group1_Ind1_Call1](./images/GroupMembership_Group1_Indiv1_Call1.png)
...
# Set all negative values to the minimum positive value in the SPCC matrix to represent the fact that these calls are very structurally different from one another
        if(neg_values2 > 0){
          ...
          # Initialize a record of what was changed to save to the final results
          neg_notes2 <- paste(round(neg_values2, 8), "% of all SPCC cells or", length(xc_mat2[xc_mat2 < 0])/2,"unique comparisons were negative and were changed to the minimum positive SPCC value of", round(min(xc_mat2[xc_mat2 > 0]), 8), sep = " ")
...
ones2 <- ((length(which(xc_mat2[lower.tri(xc_mat2)] == 1))*2)/length(xc_mat2)) * 100
        ...
        if(ones2 > 0){
          ...
          # Initialize a record of what was changed to save to the final results
          one_notes2 <- paste(round(ones2, 8), "% of all SPCC cells or", length(which(xc_mat2[lower.tri(xc_mat2)] == 1)), "unique comparisons were had an original SPCC value of 1 and were changed to the next maximum SPCC value of", round(sec_max_val2, 8), sep = " ")
...
# Save this matrix since it can take a long time to generate when there are many vocalizations
saveRDS(xc_mat2, file.path(analysis_path, "xc_mat2.RDS"))
