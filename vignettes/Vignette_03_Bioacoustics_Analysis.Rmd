...
In this vignette, we will go through how to analyze the synthetic identity signals we generated using the paRsynth package by creating spectrograms and acoustic plots comparing their acoustic similarities.
...
# Check the percentage of values that are negative
...
# Find the calls that are associated with negative values

# Take out those calls from the dataset
...
# creates a data frame which has calculation of the convex hull for each group (aka the smallest polygon that can enclose all the points in a group). This is useful for visualizing the spread of the data in the acoustic space plot (like the data istribution, boundaries in a scatter plot)
...
```{r}
# this code will create a synthetic dataset with extreme group-level variation. This dataset will have 10 times more group information than individual information

set.seed(123) # set seed for reproducibility

# setting up the parameters for the synthetic dataset
factor_ratio <- 10
...
# create the parsons code for the synthetic dataset
calls_parsons <- parsons_code(
  df = calls, 
  string_col = "Call", 
  mapping = list("A" = "up", "B" = "down", "C" = "constant")
)
# create the frequency anchors for the synthetic dataset
...
# write the audio files for the synthetic dataset
...
# write the metadata for the synthetic dataset
...
```{r}
# this code will create the selection table for the synthetic dataset with extreme group-level variation

# read the metadata for the synthetic dataset
...
```{r eval = FALSE}
# Create spectrograms for the calls in ExtraExample_sel_tbl
...
```{r}
# Perform SPCC to measure acoustic similarity of all calls in dataset ExtraExample_sel_tbl
...
str(isoEE) # checking the structure of isoEE
